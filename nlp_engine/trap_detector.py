...
class TrapDetector:
    ...
    TRAP_RULES = {
        ...
        TrapCategory.ASYMMETRIC_INDEMNIFICATION: {
            ...
            "patterns": [
                (re.compile(r"\b(?:customer|user|you|subscriber|client)\s+shall\s+(?:indemnify,\s*defend\s+and\s+hold\s+harmless|defend,\s*indemnify\s+and\s+hold\s+harmless)\b.*?\b(?:from\s+any\s+and\s+all\s+claims|against\s+all\s+losses|from\s+all\s+damages|against\s+any\s+claims)\b", re.I), 0.98),
        ...
        TrapCategory.FORCED_ARBITRATION_CLASS_WAIVER: {
            ...
            "patterns": [
                ...
                (re.compile(r"\b(?:exclusively\s+resolved\s+by\s+binding\s+arbitration|shall\s+be\s+settled\s+by\s+arbitration|mandatory\s+binding\s+arbitration)\b", re.I), 0.90),
        ...
        TrapCategory.PERPETUAL_DATA_AI_HARVESTING: {
            "severity": RiskSeverity.CRITICAL,
            "penalty": 32.0,
            "danger": "Grants the vendor rights to ingest confidential customer business data, private documents, or proprietary text to train commercial AI/LLM models.",
